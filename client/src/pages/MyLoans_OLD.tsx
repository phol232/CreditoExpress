            ...
            const loansData = await Promise.all(
                snapshot.docs.map(async (doc) => {
                    ...
                    if (scheduleSnapshot.docs.length > 0) {
                        ...
                        // Intentar obtener del documento principal si existe
                        if (data.repaymentSchedule && Array.isArray(data.repaymentSchedule)) {
                            schedule = data.repaymentSchedule.map((s: any) => ({
                                paymentNumber: s.installmentNumber || s.paymentNumber || 0,
                                dueDate: s.dueDate?.toDate() || new Date(),
                                amount: s.totalPayment || s.amount || 0,
                                principal: s.principal || 0,
                                interest: s.interest || 0,
                                balance: s.remainingBalance || s.balance || 0,
                                                    ...
                                                    {loan.repaymentSchedule.length === 0 ? (
                                                        <tr>
                                                            <td colSpan={7} className="px-4 py-8 text-center text-muted-foreground">
                                                                No hay cronograma de pagos disponible
                                                            </td>
                                                        </tr>
                                                    ) : (
